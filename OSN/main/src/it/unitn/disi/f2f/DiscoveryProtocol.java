...
class TreeState {
	...
	public boolean refresh(TreeState other) {
		return this.refresh(other.fAggregate);
	}

	private boolean refresh(BitSet state) {
		fAge = CommonState.getIntTime();
		fDirty = transfer(state, fAggregate) != 0;
		return fDirty;
	}
