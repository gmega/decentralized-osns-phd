...
/**
 * {@link EventStorage} is an auxiliary data storage structure which keeps track
 * of known/unknown events. An event is a tuple composed of: <BR>
 * 
 * <ol>
 * <li>a {@link Node}, which identifies the producer of the event</li>
 * <li>a sequence number (integer) which identifies the event uniquely in the
 * producer's event stream. Moreover, events with smaller sequence number are
 * assumed to have happened sooner than those with larger sequence number (within
 * the local event stream of a producer).</li>
 * </ol>
 * 
 * @author giuliano
 */
public class EventStorage implements Cloneable {
	...
	public List<Integer> getList(Node owner) {
	...
	/**
	 * Performs a merge of the events known by this {@link EventStorage} with
	 * the events known by some other {@link EventStorage}, constrained by a
	 * given {@link Linkable}. In essence, only those events produced by
	 * {@link Node} instances known to the {@link Linkable} will remain in the
	 * {@link EventStorage} after the operation.<BR>
	 * <BR>
	 * Note that this operation affects only this {@link EventStorage}, the
	 * other will remain unchanged (so technically it's not a merge).
	 * 
	 * @param sender
	 *            the 'sending' node; that is, the node which owns the
	 *            {@link EventStorage} object being merged into this one.
	 * @param receiver
	 *            the 'receiving' node; that is, the node which owns this
	 *            {@link EventStorage} object.
	 * @param other
	 *            the {@link EventStorage} instance with which we want to merge.
	 * @param observer
	 ...
	 * 
	 */
	public void merge(Node sender, Node receiver, EventStorage other,
		...
		while (keyIt.hasNext()) {
			...
			observer.sendDigest(sender, receiver, key, getList(key));
		...
		while (it2.hasNext()) {
			...
			while (it1.hasNext()) {
				...
				if (originalStart > end) {
					...
					observer.eventDelivered(sender, receiver, key, start,
							originalStart - 1);
				}
			...
			if (start <= end) {
				observer.eventDelivered(sender, receiver, key, start, end);
			}
		}
	...
	private void deltaAdd(Node sender, Node receiver, EventStorage other,
			IMergeObserver observer, Linkable socialNeighborhood) {
		for (Node key : other.fIntervals.keySet()) {
			...
			Iterator<Integer> it = list.iterator();
			while (it.hasNext()) {
				observer.eventDelivered(sender, receiver, key, it.next(), it
						.next());
			}
		}
	}
	...
	public Object clone() {
		try {
			EventStorage cloned = (EventStorage) super.clone();
