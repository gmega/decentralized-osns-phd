 ...
 * 
 * Slightly modified to support merges, and to dump the element counter.
 * @author giuliano
 * 
 */
public class BloomFilter<E> implements Serializable {
	...
	/**
	 * Computes the size required for the bit set size of a bloom filter if we
	 * want to achieve a target false positive probability with a target number
	 ...
	 */
    private static final double LN_OF_TWO_SQUARED = Math.pow(Math.log(2), 2.0);
    
	public static long requiredBitSetSizeFor(double p, int n) {
		return (long) -Math.ceil((n * Math.log(p))/
				(LN_OF_TWO_SQUARED));
    }
}