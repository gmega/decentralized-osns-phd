 ...
 * {@link #setConfig(Properties)}, to set configuration data. This method is
 * called by the simulator engines as the very first thing they do. It can be
 * called only once, after that the class becomes read only. All components can
 * then access this configuration and utility methods to read property values
 ...
 * and offers some utility functions. This extended functionality consists of
 * the following: reading values with type checking, ordering of entries,
 * pre-processing protocol names, parsing expressions, resolving underspecified
 * classnames, and finally some basic debugging possibilities. We discuss these
 * in the following.
 * <p>
 * Note that the configuration is initialized using a Properties object. The
 * class of this object might implement some additional pre-processing on the
 * file or provide an extended syntax for defining property files. See
 * {@link ParsedProperties} for more details. This is the class that is
 * currently used by simulation engines.
 * <h3>Typed reading of values</h3>
 * Properties can have arbitrary values of type String. This class offers a set
 * of read methods that perform the appropriate conversion of the string value
 * to the given type, eg long. They also allow for specifying default values in
 * case the given property is not specified.
 * <h3>Resolving class names</h3>
 * 
 * The possibilities for the typed reading of a value includes interpreting the
 * value as a class name. In this case an object will be constructed. It is
 * described at method {@link #getInstance(String)} how this is achieved
 * exactly. What needs to be noted here is that the property value need not be a
 * fully specified classname. It might contain only the short class name without
 * the package specification. In this case, it is attempted to locate a class
 * with that name in the classpath, and if a unique class is found, it will be
 * used. This simplifies the configuration files and also allows to remove their
 ...
 * descriptions (parameters). Although this class is blind to the semantics of
 * these components, it offers some low level functionality that helps dealing
 * with them. This functionality is based on the assumption that components have
 * a type and a name. Both types and names are strings of alphanumeric and
 * underscore characters. For example, {@value #PAR_PROT} is a type, "foo" can
 * be a name. Method {@link #getNames} allow the caller to get the list of names
 * for a given type. Some other methods, like {@link #getInstanceArray} use this
 * list to return a list of components.
 * 
 * <p>
 * Assuming the configuration is in Properties format (which is currently the
 * only format available) component types and names are defined as follows.
 * Property names containing two non-empty words separated by one dot (".")
 * character are treated specially (the words contain word characters:
 * alphanumeric and underscore ("_")). The first word will be the type, and the
 * second is the name of a component. For example,
 * 
 * <pre>
 ...
 * components not shown). When {@link #getNames} or {@link #getInstanceArray}
 * are called, eg <code>getNames("control")</code>, then the order in which
 * these are returned is alphabetical:
 * <code>["control.1","control.2","control.conn"]</code>. If you are not
 * satisfied with lexicographic order, you can specify the order in this way.
 * 
 * <pre>
 *   order.control 1,conn,2
 * </pre>
 * 
 * where the names are separated by any non-word character (non alphanumeric or
 * underscore). If not all names are listed then the given order is followed by
 * alphabetical order of the rest of the items, e.g.
 * 
 * <pre>
 *   order.control 2
 * </pre>
 ...
 * <p>
 * It is also possible to exclude elements from the list, while ordering them.
 * The syntax is identical to that of the above, only the parameter name begins
 * with <code>include</code>. For example
 * 
 * <pre>
 ...
 * <code>control.2</code>, in this order. Note that for example the empty list
 * results in a zero length array in this case. <em>Important!</em> If include
 * is defined then ordering is ignored. That is, include is stronger than order.
 * <h3>Protocol names</h3>
 * As mentioned, the configuration is generally blind to the actual names of the
 * components. There is an exception: the components of type {@value #PAR_PROT}.
 * These are pre-processed a bit to enhance performance: protocol names are
 * mapped to numeric protocol identifiers. The numeric identifier of a protocol
 * is its index in the array returned by {@link #getNames}. See above how to
 * control this order. The numeric identifiers then can be looked up based on
 * the name and vice versa. Besides, the identifier can be directly requested
 * based on a property name when the protocol name is the value of a property
 * which is frequently the case.
 * <p>
 * <h3>Expressions</h3>
 * Numeric property values can be complex expressions, that are parsed using <a
 * href="http://www.singularsys.com/jep/">JEP</a>. You can write expression
 * using the syntax that you can find <a
 ...
 * 
 * <p>
 * Expressions like "sub-expression op sub-expression" are computed based on the
 * type of the sub-expressions. If both sub-expressions are integer, the
 * computation is done using integer arithmetics and the result is an integer.
 * So, for example, 5/2 returns 2. If one of the sub-expression is floating
 * point, the computation is based on floating-point arithmetics (double
 * precision) and the result is a floating point value. So, for example, 5.0/2
 * returns 2.5.
 * 
 * <p>
 * Expressions are parsed recursively. Note that no optimization is done, so
 * expression F is evaluated three times here (due to the fact that appears
 * twice in C and once in B). But since properties are read just once at
 * initialization, this is not a performance problem.
 * 
 * <p>
 * Finally, recursive definitions are not allowed (and without function
 * definitions, they make no sense). Since it is difficult to discover complex
 * recursive chains, a simple trick is used: if the depth of recursion is
 * greater than a given threshold (configurable, currently
 * {@value #DEFAULT_MAXDEPTH}, an error message is printed. This avoids to fill
 * the stack, that results in an anonymous OutOfMemoryError. So, if you write
 * 
 * <pre>
 *   overlay.size SIZE
 *   SIZE SIZE-1
 ...
 * <h3>Debug</h3>
 * 
 * It is possible to obtain debug information about the configuration properties
 * by activating special configuration properties.
 * <p>
 * If property {@value #PAR_DEBUG} is defined, each config property and the
 * associated value are printed. Properties that are not present in the config
 * file but have default values are postfixed with the string "(DEFAULT)".
 * <p>
 * If property {@value #PAR_DEBUG} is defined and it is equal to
 * {@value #DEBUG_EXTENDED}, information about the configuration method invoked,
 * and where this method is invoked, is also printed. If it is equal to
 * {@value #DEBUG_FULL}, all the properties are printed, even if they are not
 * read.
 * <p>
 ...
 * initialized using {@link ParsedProperties}, then it is possible to use some
 * more compressed format to specify the components. See
...
public class Configuration {
	 ...
	 * configuration properties are printed at the beginning, not just when they
	 * are called.
	 * 
	 * @config
	 */
	...
	/**
	 * The type name of components describing user plugins.
	 */
	static final String PAR_PLUGIN = "plugin";
	 ...
	 * If modification is attempted, a RuntimeException is thrown and no change
	 * is made.
	 * 
	 * @param p
	 ...
	 * throws a {@link MissingParameterException}. Empty string is not accepted
	 * as false due to the similar function of {@link #contains} which returns
	 * true in that case. True is returned if the lowercase value of the
	...
	/**
	 * Calls {@link #getPid(String)}, and returns the default if no property is
	...
	/**
	 * Reads given configuration property. If not found, throws a
	 * {@link MissingParameterException}. When creating the Class object, a few
	 * attempts are done to resolve the classname. See {@link Configuration} for
	 * details.
	 * 
	 * @param name
	 *            Name of configuration property
	 ...
	 * The constructor of the class can see the configuration so it can make use
	 * of this name to read its own parameters from it.
	 * 
	 * @param name
	 *            Name of configuration property
	 ...
	 * <code>{@value #PAR_ORDER}+"."+name</code> then the order is alphabetical.
	 * Otherwise this entry defines the order. For more information see
	 * {@link Configuration}.
	 * 
	 * @param name
