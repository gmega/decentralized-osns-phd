...
/**
 * Provides static methods to obtain the package-qualified class name of a
 * class, given just the non-qualified name, and to obtain the non-qualified
 * name, given the package-qualified class name.
 * 
...
class ClassFinder {
	...
	/**
	 * The number of directories that have been touched by the search. This does
	 * not include directories in jar files.
	 */
	...
	static {
		...
		if (visitedDirs >= maxDirs) {
			System.err
					.println("Configuration: some directories in your "
	 ...
	 * "java.class.path" property are considered. Jar files and directories are
	 * both parsed. If multiple classes with the same name but different
	 * fully-qualified names are present, a comma-separated list of
	 * fully-qualified class names is returned.
	 * 
	 * @param name
	 *            the non-qualified name of the class to be searched
	...
	/**
	 * Finds all the classes reachable from the current classpath; for each of
	 * them, inserts an association (name, fully-qualified name) in the
	 * specified map. Both names are String objects.
	 * 
	 * Only classes reachable from the classpath defined by the
	 * "java.class.path" property are considered. Jar files and directories are
	 * both parsed. If multiple classes with the same name but different
	 * fully-qualified names are present, they are inserted in the map as
	 * associations (name, comma-separated list of fully-qualified names).
	 * 
	 * @param map
	 * @throws IOException
	 */
	 ...
	 * 
	 * @param map
	 *            the map where to insert associations
	 * @param pathFile
	 *            the file name of the associated jar file
	 * @throws IOException
	 */
	 ...
	 *            the map where to insert associations
	 * @param pathElement
	 *            the path string used for recursion
	 * @param pathFile
	 *            the file (directory) to be analyzed
	 * @throws IOException
	 */
	private static void findClassInPathDir(Map<String, String> map,
			String pathElement, File pathFile) throws IOException {
		visitedDirs++;
		if (visitedDirs >= maxDirs)
			return;
		...
		for (int i = 0; i < list.length; i++) {
			...
			if (file.isDirectory()) {
				...
				String classFile = file.toString().substring(
						pathElement.length());
	...
	/**
	 * Translates a class file name in a class name using the specified file
	 * separator.
	 */
	private static String classname(String classFile, String filesep) {
		return classFile.replace(filesep, ".").substring(0,
