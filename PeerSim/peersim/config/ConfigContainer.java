...
import peersim.config.resolvers.ServiceResolver;
...
public class ConfigContainer {
	...
	public ConfigContainer(Properties config, boolean check) {
		this.config = config;
		this.check = check;
		maxdepth = getInt(Configuration.PAR_MAXDEPTH,
				Configuration.DEFAULT_MAXDEPTH);

		// initialize protocol id-s
		protocols = new HashMap<String, Integer>();
		String[] prots = getNames(Configuration.PAR_PROT);// they're returned in
															// correct order
		for (int i = 0; i < prots.length; ++i) {
			protocols.put(
					prots[i].substring(Configuration.PAR_PROT.length() + 1),
					Integer.valueOf(i));
		}
	...
	public int lookupPid(String protname) {
		...
		if (ret == null) {
			throw new MissingParameterException(Configuration.PAR_PROT + "."
					+ protname);
			// "\nPossibly incorrect property: "
		...
		if (c == null) {
			...
			if (fullname != null) {
				throw new IllegalParameterException(name, "Class " + classname
						+ " does not exist. Possible candidate(s): " + fullname);
			}
		}
		if (c == null) {
			throw new IllegalParameterException(name, "Class " + classname
	 ...
	 * <code>{@value peersim.config.Configuration#PAR_INCLUDE}+"."+name</code>
	 * or <code>{@value peersim.config.Configuration#PAR_ORDER}+"."+name</code>
	 * then the order is alphabetical. Otherwise this entry defines the order.
	 * For more information see {@link Configuration}.
	 * 
	 * @param name
	 ...
	 * entry must contain entries from <code>names</code>. It is assumed that
	 * the entries in <code>names</code> contain only word characters
	...
	private String[] order(String[] names, String type) {
		...
		if (order != null && !order.equals("")) {
			...
			for (; i < sret.length; i++) {
				...
				if (j == names.length) {
					throw new IllegalParameterException(
							(include ? Configuration.PAR_INCLUDE
									: Configuration.PAR_ORDER) + "." + type,
	...
	private IResolver initializeResolvers() {
		CompositeResolver resolver = new CompositeResolver();
		resolver.addResolver(new ConfigContainerResolver(this));
	...
	private void manageDefault(String name, Object def, RuntimeException e) {
		...
		if (check) {
			System.out.println("Warning: Property " + name + " = " + def
					+ " (DEFAULT)");
		}
