...
/**
 * This OutputStream uses an underlying stream to output data. Each line
 * (terminated with `\n`) is augmented with a tag character. This is used to
 * discriminate among standard error and standard output. This feature is needed
 * for launching new JVMs; it should not be used for other purposes.
 * 
...
public class TaggedOutputStream extends PrintStream {
	...
	/**
	 * This parameter contains the string that should be printed on each line,
	 * containing the values of the range parameters for the experiment which is
	 * being run. The full name of this configuration string is prefixed by
	 * {@value peersim.Simulator#PAR_REDIRECT}.
	 * 
	 * @config
	 */
	 ...
	 * contained in parameter {@value #PAR_RANGES} should be augmented with a
	 * "TIME &lt;t&gt;" specification regarding current time. Observers are
	...
	/**
	 * Creates a tagged output stream that prints the tagged output on the
	 * specified stream.
	 */
	public TaggedOutputStream(String prefix, PrintStream oStream) {
		super(oStream);
	...
	/**
	 * Actually prints a line, inserting ranges and time when needed.
	 */
