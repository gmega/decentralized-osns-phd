...
public CDEpidemicManager(String prefix)
{
	...
	String periodString = Configuration.getString(prefix + "." + PAR_PERIOD);
	String[] protocols = protString.split(",");
	String [] periods = periodString.split(",");
	pid = new int[protocols.length];
	period = new int[protocols.length];
...
public void nextCycle(Node lnode, int protocolID)
{
	for (int i=0; i < pid.length; i++) {
		if (CommonState.getTime() % period[i] != 0) {
			continue;
		}
