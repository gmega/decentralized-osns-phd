...
/**
 * A class that implements the Kronecker product and Kronecker sum matrix
 * operations
 * 
...
public class KroneckerOperation {
	...
	public static void sum(double[] A, double[] B, double[] Im, double[] In,
			double[] tmpA, double[] tmpB, double[] C) {
		final int m = A.length;
		final int n = B.length;
		final int dim = m * n;

		if (C.length != dim || Im.length != m || In.length != n
	...
	public static void sum(double[] A, int m, double[] B, int n, double[] Im,
			double[] In, double[] tmpA, double[] tmpB, double[] C) {
	...
	public static double[] product(double[] A, int m, int n, double[] B, int p,
			int q) {
	...
	public static void product(double[] A, int m, int n, double[] B, int p,
			int q, double[] out) {
			...
			for (int j = 0; j < n; j++) { // 1,\ldots,n
				...
				for (int k = 0; k < p; k++) { // 1,\ldots,p
					for (int l = 0; l < q; l++) { // 1,\ldots,q
						out[(iOffset + k) * dimj + jOffset + l] = aij
								* B[k * q + l];
					}
				}
			}
