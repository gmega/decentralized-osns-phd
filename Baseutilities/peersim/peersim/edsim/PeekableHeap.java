 ...
 * 
 * Modified from the original PeerSim version to allow the time of the earliest
 * event to be peeked without removing it from the queue.
 * 
 * @author Alberto Montresor
 * @version $Revision: 1.10 $
 */
public class PeekableHeap implements PriorityQ {
	...
	/**
	 * This parameter specifies how many bits are used to order events that
	 * occur at the same time. Defaults to 8. A value smaller than 8 causes an
	 * IllegalParameterException. Higher values allow for a better
	 * discrimination, but reduce the maximal time steps that can be simulated.
	 * 
	 * @config
	 */
	 ...
	 */
	public PeekableHeap(String prefix) {
		...
		if (!Configuration.contains(PAR_PBITS_LEGACY))
			...
			if (Configuration.contains(prefix + "." + PAR_PBITS))
				throw new IllegalParameterException(PAR_PBITS_LEGACY,
						"Your configuration file contains both " + prefix + "."
								+ PAR_PBITS + " and " + PAR_PBITS_LEGACY
								+ "; please remove " + PAR_PBITS_LEGACY);
		}
	 ...
	 * 
	 * @param time
	 *            the time at which this event should be scheduled
	 * @param event
	 *            the object describing the event
	 * @param node
	 *            the node at which the event has to be delivered
	 * @param pid
	 *            the protocol that handles the event
	 */
	...
	public void add(long time, Object event, Node node, byte pid, long priority) {
		if ((time & overflowMask) != 0)
			throw new IllegalArgumentException("Time overflow: time=" + time);
	 ...
	 * means that data contained in the returned event are overwritten when a
	 * new invocation of this method is performed.
	 * 
	...
	/**
	 * Returns the time for the next {@link Event} returned by
	 * {@link #removeFirst()}, or {@link Long#MAX_VALUE} if the heap is empty.
	 */
	public long peek() {
		if (size == 0) {
 ...
 */
	 ...
	 * Heap heap = new Heap(); int rep = 1000000; if( args.length > 0 ) rep =
	 * Integer.parseInt(args[0]); int[] values1 = new int[rep]; long[] values2 =
	 * new long[rep]; for (int i = 0; i < rep; i++) { values1[i] =
	 * random.nextInt(1000000000); }
	 * 
	 * long time1 = System.currentTimeMillis(); for (int i = 0; i < rep; i++) {
	 * heap.add(values1[i], null, null, (byte) 1); } long time2 =
	 * System.currentTimeMillis(); System.out.println("Inserting: " +
	 * (time2-time1));
	 * 
	 * time1 = System.currentTimeMillis(); for (int i = 0; i < rep; i++) {
	 * values2[i] = heap.removeFirst().time; } time2 =
	 * System.currentTimeMillis(); System.out.println("Removing: " +
	 * (time2-time1));
	 * 
	 * Arrays.sort(values1); for (int i=0; i<rep; i++) { if (values1[i] !=
	 * values2[i]) System.out.print("+"); } }
	 */
